...
const SingleComputer = (props) => {
    const [computerProduct, getComputerProduct] = useState([]);
    ...
    return (
        ...
        <>
            ...
            { addMsgStatus  ? <CartMsg>Product {computerProduct.product_title} has been added to the cart.</CartMsg> : '' }

            <Container>
                <SProductInfo product = {computerProduct} category_header = "Computers" addToCart={ addToCart }
                getCartProductId={getCartProductId}  cartProductId={cartProductId} />
                <SProductMedia product = {computerProduct} />
            </Container>
        </>
    );
}
